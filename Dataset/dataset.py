...
def trial_analyze_video(exper_dict,exper,k,detector):
    ...
    # ignore trial if there's no realtime recording or no timestamps, don't analyse
    ...
    frames_data_test = stats_save_frames_data(video_path=vid_path,detector=detector)
    ...
    timestamps = pd.read_csv(timestamp_path,parse_dates=['0'])
    timestamps = pd.read_csv(timestamp_path,parse_dates=['0'],usecols=['0'])
    timestamps.columns = ['timestamp']
    timestamps = timestamps.values.squeeze()  # convert to numpy dt array
        
    # just in case
    if timestamps.shape[0] != exper_dict['trials'][k]['frames'].shape[0]:
        print(f'====\nWARNING: Timestamps shape incoherent with number\
        of frames at {exper} trial {k}. Ignoring trial')
        print(f"#timestamps: {timestamps.shape[0]} #frames:\
        {exper_dict['trials'][k]['frames'].shape[0]}\n=====")
        exper_dict['trials'][k]['no_realtime'] = True
        exper_dict['trials'][k]['no_screen'] = True
        exper_dict['trials'][k]['no_dlc'] = True
        return 
        
    # set timestamps as a column in dataframe
    exper_dict['trials'][k]['frames']['timestamp'] = timestamps


def trial_analyze_screen(exper_dict,exper,k):
    # parse screen touches (TODO what if file exists but no touches?)
    ...
    # TODO - maybe remove the screen dataframe, it's not needed.
    ...
    if exper_dict['trials'][k]['screen'].shape[0] == 0:
        ...
        # for each timestamp of each touch, get frame with closest timestamp
        # TODO - check distributions of deltas between touch time and frame time
        for i,ts in enumerate(exper_dict['trials'][k]['screen'].timestamp):
            frame_argmin = np.argmin((exper_dict['trials'][k]['frames']['timestamp'] \
                                      - ts).dt.total_seconds().abs())
            ...
            exper_dict['trials'][k]['frames'].iloc[frame_argmin,\
                                                   exper_dict['trials'][k]['frames']\
                                                   .columns.get_loc('hit')] = True
...
def trial_add_dlc(exper_dict,k,joints,dlc_path):
    """
...
def trial_update_dlc(exper_dict,exper,k):
    vid_time = os.listdir(os.path.join(exper,f'trial{k}','videos'))[0]
    ...
    if not os.path.exists(dlc_path):
        ...
        trial_add_dlc(exper_dict,k,DLC_JOINTS,dlc_path)
...
def analyze_experiment(exper,detector):
    """
    Recieve a path for an experiment and a detector, 
...
def analyze_new_experiments(detector,
    ...
    """
    finds epxperiments who are not analyzed yet after first date,
    """
...
def get_unified_df(all_exper,mult=False):
    """
    parse unified dataframe from all_experiments dict
