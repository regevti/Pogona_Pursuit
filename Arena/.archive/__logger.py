...
def is_always_reward():
    return cache.get(CacheColumns.IS_ALWAYS_REWARD)
...
def log(msg):
    ...
    if mqtt is not None:
        ...
        mqtt.publish(config.console_topic, msg)
...
@app.task
def end_experiment():
    cache.delete(CacheColumns.IS_VISUAL_APP_ON)
    cache.delete(CacheColumns.EXPERIMENT_BLOCK_ID)
    cache.delete(CacheColumns.EXPERIMENT_BLOCK_PATH)
    cache.delete(CacheColumns.IS_ALWAYS_REWARD)
...
def get_csv_filename(topic) -> Path:
    ...
    return Path(f'{parent}/{config.metrics_files[topic]}')
