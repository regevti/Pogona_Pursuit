...
def batch_predict_hits(f, centroids, y_threshold=930, max_timesteps):
    """
    Run the filter in order to predict the number of time steps until passing the given y coordinate
    f - The filter used for prediction. Expecting predict(), update(x) functions, and an x property.
    y_threshold - When passing this y value we assume a hit has occurred.
    max_timesteps - maximum number of time steps to predict.

    Return a data frame containing position, velocity, predicted position and number of time steps until 
    the predicted hit (or nan if there's no hit in max_timesteps steps).
    """
