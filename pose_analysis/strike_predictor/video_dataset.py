...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 is_match_annotations: bool = False):
        ...
        self.is_match_annotations = is_match_annotations
    ...
    def _get_start_indices(self, record: VideoRecord) -> 'np.ndarray[int]':
        ...
        if self.is_match_annotations:
            start_indices = np.array([record.end_frame - self.frames_per_segment])
        else:
            if self.test_mode:
                distance_between_indices = (record.num_frames - self.frames_per_segment) / float(self.num_segments)

                start_indices = np.array([int(distance_between_indices + distance_between_indices * x)
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        Returns:
            ...
            3) or anything else if a custom transform is used.
        """
        if not self.is_match_annotations:
